  ...
  async processLoginPageAsAuth(@Res() res: Response, @Body() incomingAuthDto: IncomingAuthLoginDto) {
    ...
    try {
      ...
      try {
        ...
        // send signature signing request to user
        const data = await axios.default.get('http://localhost:8002/send_request');
        return res.redirect('http://localhost:8002/verify_request');
      } catch (e) {
        /**
